...
# ---------------------------------------------------------------------------
# find_duplicate_candidates — composite over primitives
# ---------------------------------------------------------------------------


def _post(id_, title, **extra_attrs):
    attrs = {"title": title}
    attrs.update(extra_attrs)
    return {"id": str(id_), "type": "job-post", "attributes": attrs}


def _company(id_, name):
    return {"id": str(id_), "type": "company", "attributes": {"name": name}}


def _fake_api(responses):
    """Build a fake ApiClient whose get_data dispatches by URL path + filter.

    `responses` is a dict mapping (path, filter-fragment) → (payload, error, status).
    The fragment is matched as a substring of the urlencoded params so callers
    can key on `filter[link]=...` or `filter[company_id]=...` without writing
    out the whole query string.
    """
    async def _get_data(path, params=None):
        params = params or {}
        for (want_path, fragment), result in responses.items():
            if want_path != path:
                continue
            joined = "&".join(f"{k}={v}" for k, v in params.items())
            if fragment in joined or fragment == "":
                return result
...
class TestFindDuplicateCandidates:
    ...
    async def test_title_required(self):
        out = yaml.safe_load(await find_duplicate_candidates(_fake_api({}), title=""))
        assert "error" in out and "title" in out["error"]

    @pytest.mark.asyncio
    async def test_neither_link_nor_company_errors(self):
        out = yaml.safe_load(
            await find_duplicate_candidates(_fake_api({}), title="Senior Engineer")
        )
        assert "error" in out
        assert "link" in out["error"] and "company" in out["error"]

    @pytest.mark.asyncio
    async def test_link_match_is_high_confidence(self):
        api = _fake_api({
            ("/api/v1/job-posts/", "filter[link]"): (
                {"data": [_post(42, "Senior Visa Engineer")]}, None, 200,
            ),
        })
        out = yaml.safe_load(await find_duplicate_candidates(
            api, title="Senior Visa Engineer", link="https://example.com/job/42",
        ))
        assert out["count"] == 1
        cand = out["candidates"][0]
        assert cand["id"] == "42"
        assert cand["confidence"] == "high"
        assert cand["match_signals"] == ["link"]
        assert cand["frontend_url"] == "/job-posts/42"

    @pytest.mark.asyncio
    async def test_company_exact_title_is_high_confidence(self):
        api = _fake_api({
            ("/api/v1/companies/", "filter[query]"): (
                {"data": [_company(7, "Visa")]}, None, 200,
            ),
            ("/api/v1/job-posts/", "filter[company_id]=7"): (
                {"data": [_post(91, "Senior Visa Engineer")]}, None, 200,
            ),
        })
        out = yaml.safe_load(await find_duplicate_candidates(
            api, title="Senior Visa Engineer", company="Visa",
        ))
        assert out["count"] == 1
        cand = out["candidates"][0]
        assert cand["company_name"] == "Visa"
        assert cand["confidence"] == "high"
        assert "title_exact" in cand["match_signals"]

    @pytest.mark.asyncio
    async def test_company_prefix_overlap_is_medium(self):
        api = _fake_api({
            ("/api/v1/companies/", "filter[query]"): (
                {"data": [_company(7, "Visa")]}, None, 200,
            ),
            ("/api/v1/job-posts/", "filter[company_id]=7"): (
                {"data": [
                    _post(100, "Engineer"),
                    _post(101, "Marketing Lead"),
                ]}, None, 200,
            ),
        })
        out = yaml.safe_load(await find_duplicate_candidates(
            api, title="Engineer II", company="Visa",
        ))
        # Only id=100 overlaps as a prefix; 101 has no overlap.
        assert out["count"] == 1
        cand = out["candidates"][0]
        assert cand["id"] == "100"
        assert cand["confidence"] == "medium"
        assert cand["match_signals"] == ["title_similarity"]

    @pytest.mark.asyncio
    async def test_unknown_company_returns_empty(self):
        api = _fake_api({
            ("/api/v1/companies/", "filter[query]"): (
                {"data": []}, None, 200,
            ),
        ...
        out = yaml.safe_load(await find_duplicate_candidates(
            api, title="Anything", company="NeverHeardOf",
        ))
        assert out == {"candidates": [], "count": 0}

    @pytest.mark.asyncio
    async def test_signals_merge_across_strands(self):
        # Same post id hits via both link AND company+exact-title — expect
        # both signals to merge on one entry and confidence to stay 'high'.
        api = _fake_api({
            ("/api/v1/job-posts/", "filter[link]"): (
                {"data": [_post(55, "Senior Visa Engineer")]}, None, 200,
            ),
            ("/api/v1/companies/", "filter[query]"): (
                {"data": [_company(7, "Visa")]}, None, 200,
            ),
