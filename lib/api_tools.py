...
async def create_job_post_with_company_check(
    ...
    try:
        # Deliberately no client-side duplicate gate: the api owns job
        # identity, canonicalizes the link, and reconciles on write. A probe
        # from here only ever sees an exact `link` match, so blocking the
        # POST on it suppresses the reconciliation the api would have done.
