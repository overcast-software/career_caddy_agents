...
|---|---|---|---|---|---|---|
| Public MCP | `public_server.py` | SSE | `:8030` (prod), `:8000` (local) | Per-client `jh_*` API key (read-only) | **Prod** at `careercaddy.online/mcp` (entrypoint `caddy-public`) | External MCP clients (Claude Desktop, cc_auto, Cursor, etc.) |
